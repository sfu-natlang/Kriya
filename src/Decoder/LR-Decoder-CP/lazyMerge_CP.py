...
class Lazy(object):
    ...
    def setSourceInfo(self, cube_indx, src_rule, spanT, cube_depth_hier, candRefsLst=[], isSet=False):
        cube_obj = self.__getObject(cube_indx)
        Cube.setSourceSide(cube_obj, src_rule, spanT, cube_depth_hier, isSet)
    ...
    def mergeProducts(self):
        ...
        for cube_indx, cube_obj in self.cubeDict.iteritems():
            ...
            if self.cbp_heap_diversity > 1:
                diversityItems = []
                diversityItems = Cube.getkItems4Diversity(cube_obj, cube_indx, self.cbp_heap_diversity-1, True)
		...
		##TODO: should we add the uncovered spans to target to make it unique?!
            ...
            for mP_candTup in neighbours:
                ...
                if new_entry_obj is None:
                ...
                else:##TODO: should we add the uncovered spans to target to make it unique?!
                    entry_exists = Lazy.checkHypothesis(new_entry_obj.sign)
                    if entry_exists is None:                        # New hypothesis; add to heap directly
                        ...
                        curr_h_indx = self.getItemIndxInHeap(entry_exists)
                        score_w_lmHeu = Entry.recombineEntry(new_entry_obj, self.coverageHeap[curr_h_indx][2], Lazy.wvec_lm)	##score with fc
                        new_candTup = (-score_w_lmHeu, heap_indx, new_entry_obj, mP_candTup[2], mP_candTup[3])
                    else:
                        curr_h_indx = self.getItemIndxInHeap(entry_exists)
                        score_w_lmHeu = Entry.copyLMInfo(new_entry_obj, self.coverageHeap[curr_h_indx][2], Lazy.wvec_lm)	##score with fc
                        new_candTup = (-score_w_lmHeu, heap_indx, new_entry_obj, mP_candTup[2], mP_candTup[3])		
...
class Cube(object):
    __slots__ = "dimensions", "src_side", "spanT", "ruleLst", "initDimVec", "trackCubeDict", "depth_hier", "cbp_heap_diversity", "LMCache", "setCube", "boundLst"
    ...
    def __del__(self):
        ...
        del self.LMCache
        if self.setCube: del self.boundLst

    def setSourceSide(self, src_rule, spanTup, cube_depth_hier, isSet=False):
    ...
    def getkItems4Diversity(self, cube_indx, k, heap_div=False):
    ...
    def mergeEntries(self, entriesLst, cube_indx):
        ...
        for ent_obj in entriesLst:
            ...
            if dim == 0:	## order is important, 0 : hypotheses list
                #score = ent_obj.score - ent_obj.lm_heu #we have not added lm_heu in score
                score = ent_obj.score
                fVec = ent_obj.featVec[:]
                cons_item = ConsequentItem(ent_obj.tgt, ent_obj.tgt_elided, ent_obj.lm_right)
                filled = set([i for i  in range(ent_obj.unc_spans[0][0][0], ent_obj.unc_spans[0][0][1])])
                cover = ent_obj.cover.copy()
                if settings.opts.hyp_sign == 2: tmpSpanLst = list(ent_obj.sign[1][1:])
            else:
                ...
                filled = filled - ent_obj.cover
                if settings.opts.hyp_sign == 2:
                    for l in ent_obj.unc_spans:
                        tmpSpanLst.append((l[0][0],l[0][1]))
            dim += 1
