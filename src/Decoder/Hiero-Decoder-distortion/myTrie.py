...
class SuffixTreeNode(object):
    ...
    def dfSearch(self, root_node, wLst):
        ...
        while len(dfsStack) > 0:
            ...
            if curr_label == "X__1" or curr_label == "X__2": prev_match = search_indx - curr_node_depth + 1
            else: prev_match = 0

            # a non-terminal X__1 or X__2 is found in the current node
            if search_label != "$$$" and (curr_node.__childIndxDict.has_key("X__1") or curr_node.__childIndxDict.has_key("X__2")):

                # either X__1 or X__2 is the child of the current node
                spanIndxLst = sIndxLst[:]
                spanIndxLst.append( search_indx )
                for child in ["X__1", "X__2"]:
                    if not curr_node.__childIndxDict.has_key(child): continue
            ...
            # non-terminal X__1 or X__2 continues for the current position in the given pattern; or
