...
my $multi_objective_style = undef;
my $comb_wgts = undef;
my $how_many_metrics = 2;
my $what_metrics = "bleu:ter";
my @metrics = qw(bleu ter);
my $optimize_metric = "bleu"; # what metric should PRO use?
...
GetOptions(
  "max-runs=i" => \$max_runs,
  "opt-metric=s" => \$optimize_metric, #passed to PRO
  ...
  "average" => \$___AVERAGE,
  "closest" => \$___CLOSEST,
  "multi-obj-style=s" => \$multi_objective_style,
  "metrics-count=i" => \$how_many_metrics,
  "metrics-list=s" => \$what_metrics,
...
Options:
  ...
  --closest ... Use either closest or shortest (default) reference
                  length as effective reference length
  --multi-obj-style=STRING ... Multi-objective tuning style 
                'joint' - include samples and tune for all at the same time
                'iterate' - tune towards one at a time and iterate metrics
                'pareto' - tune towards increasing pareto points (Duh et al. 12)
                'linear-comb' - tune linear combination baseline (baseline for Duh et al. 12)
  --metrics-count=INT ... Number of metrics for multi-objective tuning (default 2)
  --comb-wgts ... combination weights (colon separated) for the linear combination and pareto style multi-objective
  --metrics-list=STRING ... ':' separated list of metrics for multi-objective tuning (default BLEU and TER)
...
my $mert_mert_cmd = $multi_objective_style eq "joint" ? "$mertdir/PRO_joint.py" : "$mertdir/PRO.py";
...
die "Set --comb-wgts (colon separated weights) options for linear-combination. Exiting!!\n" if ($multi_objective_style =~ /linear-comb|pareto/ && !defined $comb_wgts);

my @metricsArr = ();
if ($how_many_metrics > 1) {
    foreach my $metric ( split(/:/, $what_metrics) ) {
        if (lc($metric) eq "bleu") { push @metricsArr, "BLEU"; }
        elsif (lc($metric) =~ /bleu(\d)/) { push @metricsArr, "BLEU$1"; }
        elsif (lc($metric) =~ "meteor") { push @metricsArr, "METEOR"; }
        elsif (lc($metric) =~ "ribes") { push @metricsArr, "RIBES"; }
        elsif (lc($metric) =~ "ter") { push @metricsArr, "TER"; }
        else { die "Metric $metric is not supported yet. Exiting!\n"; }
    }
}
...
if ($mert_status != 100) {
    ...
    my $curr_metric_indx = $run % scalar(@metricsArr);
    for(my $i = 0; $i <= $curr_metric_indx; $i++) {
        $optimize_metric = shift(@metricsArr);
        push @metricsArr, $optimize_metric;
    }
}
else {
    $run=$start_run-1;
...
# open local scope
{
    ...
    while(1) {
        ...
        print "  INFO: Tuning for $optimize_metric in this iteration ...\n";
        ...
        if ($multi_objective_style eq "linear-comb") { $metric_score_file = "$___WORKING_DIR/Metrics-stats/lincomb.out.lincomb"; }
        elsif ($multi_objective_style eq "pareto") { $metric_score_file = "$___WORKING_DIR/Metrics-stats/pareto.out.pareto"; }
        elsif ($optimize_metric =~ /bleu(\d)/i) { $metric_score_file = "$___WORKING_DIR/Metrics-stats/bleu$1.out.bleu$1"; }
        elsif ($optimize_metric =~ /bleu/i) { $metric_score_file = "$___WORKING_DIR/Metrics-stats/bleu.out.bleu"; }
        elsif ($optimize_metric =~ /meteor/i) { $metric_score_file = "$___WORKING_DIR/Metrics-stats/meteor.out.meteor"; }
        elsif ($optimize_metric =~ /ribes/i) { $metric_score_file = "$___WORKING_DIR/Metrics-stats/ribes.scores.ribes"; }
        elsif ($optimize_metric =~ /ter/i) { $metric_score_file = "$___WORKING_DIR/Metrics-stats/ter.scores.ter"; }
        ...
        if ($mert_status !~ /12$/) {
            ...
            if (scalar(@metricsArr) > 1) {
                $cmd .= " --multi-obj --multi-obj-style=$multi_objective_style --metric=$what_metrics";
                $cmd .= " --comb-wgts=$comb_wgts" if (defined $comb_wgts);
            }
            else { $cmd .= " --metric=$optimize_metric"; }
            if (defined $___JOBS) {
        ...
        # run PRO
        $cmd = "/home/baskaran/software/LANG/PYTHON/2.7.3/bin/python $mert_mert_cmd $run $___WORKING_DIR $ref_abs";
        if ($multi_objective_style eq "pareto") { $cmd .= " $metric_score_file $pro_sampler pareto"; }
        elsif ($multi_objective_style eq "iterate") { $cmd .= " $metric_score_file $pro_sampler $optimize_metric"; }
        elsif ($multi_objective_style eq "joint") {
            my @metricScoreFiles = ();
            foreach my $metric ( split(/:/, $what_metrics) ) {
                if ($metric =~ /bleu(\d)/i) { push @metricScoreFiles, "$___WORKING_DIR/Metrics-stats/bleu$1.out.bleu$1"; }
                push @metricScoreFiles, "$___WORKING_DIR/Metrics-stats/bleu.out.bleu" if (lc($metric) eq "bleu");
                push @metricScoreFiles, "$___WORKING_DIR/Metrics-stats/meteor.out.meteor" if (lc($metric) eq "meteor");
                push @metricScoreFiles, "$___WORKING_DIR/Metrics-stats/ribes.scores.ribes" if (lc($metric) eq "ribes");
                push @metricScoreFiles, "$___WORKING_DIR/Metrics-stats/ter.scores.ter" if (lc($metric) eq "ter");
            }
            my $met_files = join(":", @metricScoreFiles);
            $cmd .= " $met_files $pro_sampler $what_metrics";
        }
        elsif ($multi_objective_style eq "linear-comb") { $cmd .= " $metric_score_file $pro_sampler linear-comb"; }
        else { $cmd .= " $metric_score_file $pro_sampler $what_metrics"; }
        if (defined $___JOBS) {
        ...
        # Set the optimizer metricmetric  for next iteration
        $optimize_metric = shift(@metricsArr);
        push @metricsArr, $optimize_metric;
    }
