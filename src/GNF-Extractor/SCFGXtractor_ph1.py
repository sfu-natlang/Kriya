...
def iterateInitPhrPairs(iIPP_phr_len, iIPP_s_span, iIPP_t_span):
    global tot_rules_derived
    iIPP_rule = ''
    iIPP_rule = (iIPP_s_span, iIPP_t_span)
    #xtractGlueRules(iIPP_rule)
    for iIPP_sub_phr_len in range(iIPP_phr_len-1, 0, -1):
		...
		# modified
                ...
                tgt_side_compatible = checkRuleCompatibilityforSrc(iIPP_t_span, iIPP_sub_phr_pair[1])
...
def checkRuleCompatibilityforTgt(cRC_rule, cRC_sub_rule):
    ...
    #left_to_right_pattern = cRC_sub_rule + "\s?X__1"
    #if not cRC_rule.startswith(cRC_r_pad):
     #   if re.search("X__[1|2]",cRC_rule) is None:
#		if cRC_rule.endswith(cRC_l_pad):
#	        	cRC_rule_compatible = True
	#elif re.search(left_to_right_pattern,cRC_rle) is not None:
	 #       cRC_rule_compatible = True
