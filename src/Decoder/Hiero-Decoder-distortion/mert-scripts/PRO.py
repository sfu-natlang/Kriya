...
def execPRO(run_id, what_metric, workDir, refFile, metricScoreFile):
    ...
    for p_iter in xrange( PRO_iters ):
        sent_indx = 0
        start_indx = 0
        feat_offset = 0
        ...
        while sent_indx < tune_set_size:
            ...
            metricScores = loadMetricStats(metricScoreFile, what_metric, start_indx, cands_4_sent)
            if what_metric == 'pareto' or SAMPLER_TYPE == 'rank':
            ...
            for featVec in hypData:
                ...
                for f_val in featVec:
                    if f_val != 0.0:
                        if what_metric == 'pareto' or SAMPLER_TYPE == 'rank' or SAMPLER_TYPE == 'none':
                            featsLst.append( str(j+1) + ':' + str(f_val) )
                        else: featsLst.append( featNames[j] + ' ' + str(f_val) )
                    j += 1
                if what_metric == 'pareto' or SAMPLER_TYPE == 'rank' or SAMPLER_TYPE == 'none':
                    oF.write( "%g qid:%d %s\n" % (hypVal[i], sent_indx+1, ' '.join(featsLst)) )
        ...
        #finalWgts = normalizeWgts( interpolateWgts(prevWgtVec, wgtVec) )
        finalWgts = normalizeWgts(wgtVec)
        prevWgtVec = finalWgts[:]
        #writeFinalWgts(prevWgtVec, wgtsFile)

    newWgtsFile = workDir + "weights.txt"
    writeFinalWgts(prevWgtVec, newWgtsFile)

    logFile = workDir + "pro.log"
    bestHypIndxLst = identifyBestHyps(prevWgtVec, featFile)
...
def main():
    ...
    if len(sys.argv) == 7:
        gold_met = sys.argv[6].lower()
        if (not (gold_met == 'pareto' or gold_met == 'linear-comb' or gold_met.startswith('bleu') or \
                    gold_met == 'meteor' or gold_met == 'ribes' or gold_met == 'ter')):
            sys.stderr.write("Only Pareto (PMO), Linear-comb, BLEU, BLEUn, METEOR, RIBES and TER are supported currently as gold metrics\n")
            sys.exit(1)

    if not workDir.endswith("/"): workDir += "/"
    sys.stderr.write("Using %s sampler for sampling candidate pairs\n" % (SAMPLER_TYPE))
    sys.stderr.write("Using %s as gold metric for optimization\n" % (gold_met))

    t_beg = time.time()
    execPRO(run_id, gold_met, workDir, refFile, metricScoreFile)
    t_end = time.time()
    sys.stderr.write("Total PRO iterations : %d\n" % (PRO_iters))
