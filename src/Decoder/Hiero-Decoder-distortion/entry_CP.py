...
class Entry(object):
    ...
    def expandEntry(self, other, span, spanLst=()):
	other = self.copyEntry(other, span)
	start = span[0]		
	end = span[1]+1
        distortion = 0
        X_1_tgt = other.tgt.find('X__1')
        tgt_end_ind = len(other.tgt)

        if X_1_tgt >= 0:
	        spans = list(spanLst)
                X1=abs(spans[0][1]-spans[0][0]+1)
                tgt_end_ind = X_1_tgt
                X_2_tgt = other.tgt.find('X__2')
                if X_2_tgt >= 0:
                        X2=abs(spans[1][1]-spans[1][0]+1)
                        T1=abs(spans[1][0]-spans[0][1]-1)
                        T2=abs(end-spans[1][1]-1)
                        if (X_1_tgt - X_2_tgt) > 0:
                                if other.featVec[-1] != -1:
					sys.stderr.write("ERR: in computing reorder")
					other.printEntry()
					sys.exit(1)
                                tgt_end_ind = X_2_tgt
                        ...
                        T1=abs(end-spans[0][1]-1)
                        if T1 >0: distortion = X1+T1

        other.featVec[-2] = -distortion
	other.score += -distortion * settings.opts.weight_d
        return other
    ...
    def printEntry(self):
        ...
        if (settings.opts.no_glue_penalty):
            ...
            feats = ['lm:', 'glue:', 'wp:', 'tm:', 'd:', 'r:']

        tm_str = ' '.join( map(lambda x: str(x), self.featVec[0:5]) )
        if (settings.opts.zmert_nbest):
            feat_str = ' '.join( map(lambda x: str(x), [lm_excl_UNK, self.featVec[5], tm_str, self.featVec[8], self.featVec[9]]) ) if (settings.opts.no_glue_penalty) \
                        else ' '.join( map(lambda x: str(x), [lm_excl_UNK, self.featVec[7], self.featVec[5], tm_str, self.featVec[8], self.featVec[9]]) )
        else:
            feat_str = ' '.join( map(lambda x,y: x+' '+str(y), feats, [lm_excl_UNK, self.featVec[5], tm_str, self.featVec[8], self.featVec[9]]) ) if (settings.opts.no_glue_penalty) \
                        else ' '.join( map(lambda x,y: x+' '+str(y), feats, [lm_excl_UNK, self.featVec[7], self.featVec[5], tm_str, self.featVec[8], self.featVec[9]]) )
