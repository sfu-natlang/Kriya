...
# open local scope
{
    ...
    while(1) {
        ...
        if ($mert_status !~ /12$/) {
            ...
            $cmd = "/usr/bin/env perl $mert_extract_cmd --refs=".join(",", @references)." --run=$run --work-dir=$___WORKING_DIR --metric=$pro_opt_metric";
            if (defined $___JOBS) {
        ...
        # run PRO
        $cmd = "/home/msiahban/Modules/PYTHON/2.7.3/bin/python $mert_mert_cmd $run $___WORKING_DIR $ref_abs $metric_score_file $pro_sampler $pro_opt_metric";
        if (defined $___JOBS) {
...
sub xtract_past_stats {
    ...
    for ($r=1; $r<=$prev_run; $r++) {
        ...
        $cmd = "/usr/bin/env perl $mert_extract_cmd --refs=".join(",", @references)." --run=$run --work-dir=$___WORKING_DIR";
