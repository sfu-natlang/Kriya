...
# uses ABBR names.
my $ABBR_FULL_MAP = "lm=weight_lm tm=weight_tm wp=weight_wp glue=weight_glue";
...
my $step_runs = 0; # no of runs for each MERT step (default 0 indicates MERT run till convergence)
...
Options:
  ...
  --step-runs=N ... set the no of MERT runs (for running MERT in steps with increasing input sizes)
...
while(1) {
  ...
  $cmd = "$mert_extract_cmd $mertargs --scfile $score_file --ffile $feature_file  -r $___DEV_E -n $nbest_file";
 ...
 # backup copies
  ...
  if ($shouldstop || ($step_runs > 0 && $run == $step_runs)) {
    if ($run == $step_runs) {
      print STDERR "Completed $step_runs runs of MERT. Stopping run for the current step.\n";
    }
    else{
...
sub run_decoder {
    ...
    if (defined $___JOBS) {
        if ( -f $___SCFG_RULES ) {              # For specifying the ttable-file directly
	        $decoder_cmd = "$moses_parallel_cmd $pass_old_sge -config $___CONFIG -qsub-prefix mert$run -queue-parameters \"$queue_flags\" -mert-status $mert_status -prev-jobid $prev_jobid -n-best-list $filename -n-best-size $___N_BEST_LIST_SIZE -input-dir $___DEV_F -output-dir $___WORKING_DIR --ttable-file $___SCFG_RULES --lmfile $___LM_FILE -jobs $___JOBS -decoder-prefix $___DECODER_PREFIX -decoder $___DECODER -nodes-prop \"$nodes_prop\" > run$run.out";
        }
        elsif ( -d $___SCFG_RULES ) {           # For specifying the rule-dir having the ttable-files
	        $decoder_cmd = "$moses_parallel_cmd $pass_old_sge -config $___CONFIG -qsub-prefix mert$run -queue-parameters \"$queue_flags\" -mert-status $mert_status -prev-jobid $prev_jobid -n-best-list $filename -n-best-size $___N_BEST_LIST_SIZE -input-dir $___DEV_F -output-dir $___WORKING_DIR -rule-dir $___SCFG_RULES --lmfile $___LM_FILE -jobs $___JOBS -decoder-prefix $___DECODER_PREFIX -decoder $___DECODER -nodes-prop \"$nodes_prop\" > run$run.out";

            #$decoder_cmd = "$moses_parallel_cmd $pass_old_sge -config $___CONFIG -qsub-prefix mert$run -queue-parameters \"$queue_flags\" -decoder-parameters \"$parameters $decoder_config\" -mert-status $mert_status -prev-jobid $prev_jobid -n-best-list $filename -n-best-size $___N_BEST_LIST_SIZE -input-dir $___DEV_F -output-dir $___WORKING_DIR -rule-dir $___SCFG_RULES --lmfile $___LM_FILE -jobs $___JOBS -decoder-prefix $___DECODER_PREFIX -decoder $___DECODER -nodes-prop \"$nodes_prop\" > run$run.out";
        }
    } else {
      my $sentIndex = 100;
      my $in_file = $___DEV_F;
      ...
      $decoder_cmd = "$___DECODER_PREFIX $___DECODER $parameters --config $___CONFIG --index $sentIndex --inputfile $in_file --outputfile $filename --glue-file $___GLUE_RULES --ttable-file $rule_file --lmodel-file $___LM_FILE > run$run.out";
